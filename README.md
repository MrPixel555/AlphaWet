...
- measures real latency to `google.com` through the active local Xray HTTP proxy
...
Then run `tool/prepare_android_runtime.sh`. The script copies those source binaries into `android/app/src/main/jniLibs/<abi>/libxraycore.so`, so Android installs them into `ApplicationInfo.nativeLibraryDir` where they can be executed on Android 10+.
