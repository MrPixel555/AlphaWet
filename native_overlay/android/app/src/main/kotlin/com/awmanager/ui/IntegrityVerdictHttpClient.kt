        ...
        try {
            ...
            if (statusCode !in 200..299) {
                ...
                throw SecurityException(
                    "Integrity verdict endpoint rejected the request "
                        + "(status=$statusCode, elapsedMs=$elapsed, viaProxy=$useActiveTunnelProxy, proxyHttpPort=$proxyHttpPort). "
                        + responseText,
                )
            }
            ...
            val elapsed = System.currentTimeMillis() - startedAt
            throw SecurityException(
                "Integrity verdict decode request failed after ${elapsed} ms "
                    + "(viaProxy=$useActiveTunnelProxy, proxyHttpPort=$proxyHttpPort, verdictUrl=$verdictUrl): "
